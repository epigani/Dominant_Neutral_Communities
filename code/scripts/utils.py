...
def maximum_likelihood_exponent(data, xmin=1):
    '''
    ...
    '''
    rho = 1/np.mean(np.log(data[data>=xmin]/xmin))
    # calculate the p-value of the Kolmogorov-Smirnov test
    def power_law_cdf(x, rho, xmin):
        return 1 - (x / xmin)**(- rho)
    
    if xmin != 1:
        KS = kstest(data, lambda x: power_law_cdf(x, rho, xmin))
    else:  
